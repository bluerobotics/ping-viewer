...
void Test::fileManager()
{
    ...
    for (const auto& folder : fileManager->folderMap) {
        QVERIFY2(folder->dir.exists(), qPrintable(QString("Folder does not exist: %1").arg(folder->dir.path())));
        ...
        QVERIFY2(folder->ok, qPrintable(QString("Folder exist but ok is invalid: %1").arg(folder->dir.path())));
    }
    ...
    for (const auto key : fileManager->folderMap.keys()) {
        ...
        QVERIFY2(
            match.hasMatch(), qPrintable(QString("File name does not follow rules of FileManager: %1").arg(fileName)));
...
void Test::logger()
{
    ...
    logger->logMessage("This is the result of our test build!", QtMsgType::QtDebugMsg,
...
void Test::ringVector()
{
    ...
    for (int i {0}; i < ring.length(); i++) {
        QVERIFY2(ring[i] == size - 1 - i, qPrintable(QString("Ring is not working: Ring[%2]=%1").arg(ring[i]).arg(i)));
    }
}
...
void Test::settingsManager()
{
    ...
    QVERIFY2(index == 0, qPrintable(QString("Meters is not default: %1").arg(index)));
    auto name = settingsManager->distanceUnits()[QString("name")].toString();
    QVERIFY2(name.contains("Metric"), qPrintable(QString("Distance unit name is wrong: %1").arg(name)));
    auto scalar = settingsManager->distanceUnits()[QString("distanceScalar")].toDouble();
    QVERIFY2(qFuzzyCompare(scalar, 1), qPrintable(QString("Distance scalar in meters is wrong: %1").arg(scalar)));

    // Change to imperial
    settingsManager->distanceUnitsIndex(1);
    index = settingsManager->distanceUnitsIndex();
    QVERIFY2(index == 1, qPrintable(QString("Not changed to impérial: %1").arg(index)));
    name = settingsManager->distanceUnits()[QString("name")].toString();
    QVERIFY2(name.contains("Imperial"), qPrintable(QString("Distance unit name is wrong: %1").arg(name)));
    scalar = settingsManager->distanceUnits()[QString("distanceScalar")].toDouble();
    QVERIFY2(
        qFuzzyCompare(scalar, 3.280839895), qPrintable(QString("Distance scalar in meters is wrong: %1").arg(scalar)));
...
void Test::waterfallGradient()
{
    ...
    QVERIFY2(color0 == QColor(Qt::black), qPrintable(QString("Color does not match: %1").arg(color0.name())));

    QVERIFY2(color05 == QColor("#7f7f7f"), qPrintable(QString("Color does not match: %1").arg(color05.name())));

    QVERIFY2(color1 == QColor(Qt::white), qPrintable(QString("Color does not match: %1").arg(color1.name())));

    float value0 = gradient.getValue(color0);
    float value05 = gradient.getValue(color05);
    float value1 = gradient.getValue(color1);

    QVERIFY2(qFuzzyCompare(value0, 0), qPrintable(QString("Value does not match: %1").arg(value0)));

    QVERIFY2(abs(value05 - 0.5) < 0.05, qPrintable(QString("Value does not match: %1").arg(value05)));

    QVERIFY2(qFuzzyCompare(value1, 1), qPrintable(QString("Value does not match: %1").arg(value1)));
}
